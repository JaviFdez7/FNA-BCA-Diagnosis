...
El dataset cuenta con 33 columnas y 569 filas.
...
Aunque más adelante se volverá a pasar a categórica, ahora es necesario hacerla binaria para poder estudiar la correlación de las variables.
...
A continuación, se verifica que todas las columnas del dataset sean numéricas, ya que algunos modelos de aprendizaje automático requieren que todas las variables de entrada sean numéricas. Para ello, se identifican las columnas categóricas y se comprueba que todas las columnas sean numéricas.
...
Al tener el dataset limpio de NA y solo presentes variables numéricas, el siguiente paso será estudiar las posibles correlaciones de nuestro dataset.
...
Se evaluaron diferentes modelos de clasificación para un conjunto de datos binario con 569 muestras y 30 predictores, categorizados en 'Negative' y 'Positive'. Los modelos analizados incluyen el Árbol de Decisión (CART), Random Forest (Bosque Aleatorio), Máquinas de Soporte Vectorial con núcleo Radial (SVM), Modelo Lineal Generalizado (GLM) y un clasificador con redes neuronales.
...
Establecemos una semilla para asegurar que los resultados sean reproducibles, es decir, que los mismos resultados se obtengan cada vez que se ejecute el código. Luego, configuramos el K-fold Cross-Validation con probabilidades de clase, que nos permitirá evaluar los modelos utilizando métricas de clasificación binaria.
...
cat("Valor del AUC-ROC para el baseline:", auc_value, "\n")
...
#### 3.3.1. Evaluación de los modelos
...
#### 3.3.2. Estudio de la Importancia de las Variables
...
##### 3.3.2.1. Árbol de Decisión (CART)
...
#### 3.3.4. Comparación de resultados análisis supervisado dataset incial vs dataset reducido:
...
### 4. Conclusiones
Hemos realizado un análisis completo de un conjunto de datos de cáncer de mama, incluyendo la exploración de datos, la construcción de modelos de aprendizaje supervisado y no supervisado, y la identificación de reglas de asociación. Algunos de los hallazgos clave incluyen:
Detección de patrones de agrupación en los datos de cáncer de mama mediante técnicas de clustering y reducción de dimensionalidad. En los que 2 de los 4 clústeres identificados en el análisis no supervisado son 100% malignos, por lo que podrían ser de especial interés para futuros estudios.
Identificación de reglas de asociación significativas entre las variables del conjunto de datos, lo que proporciona información valiosa sobre las relaciones entre las características de las células y el diagnóstico de cáncer de mama.
Respecto al aprendizaje supervisado, hemos evaluado varios modelos, incluyendo Árboles de Decisión, Random Forest, SVM, Regresión Logística y Redes Neuronales. Los modelos Random Forest y SVM mostraron un rendimiento excepcional, con altas puntuaciones de AUC y sensibilidad. La reducción de variables menos importantes no afectó significativamente el rendimiento de los modelos, lo que sugiere que las variables eliminadas no aportaban información relevante pero tampoco introducían ruido probablemente debido a la alta correlación entre las variables.
En general, este análisis proporciona información valiosa para la detección y el diagnóstico del cáncer de mama, y destaca la importancia de utilizar una variedad de técnicas de análisis de datos para obtener una comprensión completa de los patrones y relaciones en los datos.
